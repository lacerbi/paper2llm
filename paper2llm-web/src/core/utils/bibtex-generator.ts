 ...
 */
function generateBibTeXKey(title: string, authors: string[], year?: string): string {
...
function formatBibTeXEntry(entry: BibTeXEntry): string {
  ...
  if (entry.authors && entry.authors.length > 0) {
    const authorStr = entry.authors.map(author => sanitizeBibTeXText(author)).join(' and ');
  ...
  if (entry.journal) bibtex += `  journal = {${sanitizeBibTeXText(entry.journal)}},\n`;
  ...
  if (entry.publisher) bibtex += `  publisher = {${sanitizeBibTeXText(entry.publisher)}},\n`;
  if (entry.doi) bibtex += `  doi = {${entry.doi}},\n`;
  if (entry.url) bibtex += `  url = {${entry.url}},\n`;
  if (entry.abstract) bibtex += `  abstract = {${sanitizeBibTeXText(entry.abstract)}},\n`;
  
  // Add any additional fields
  for (const [key, value] of Object.entries(entry)) {
    const skipFields = ['key', 'type', 'title', 'authors', 'year', 'journal', 
                         'volume', 'issue', 'pages', 'publisher', 'doi', 'url', 'abstract'];
    if (!skipFields.includes(key) && value !== undefined) {
      const formattedValue = typeof value === 'string' ? sanitizeBibTeXText(value) : value;
 ...
 */
function generateMockBibTeXEntry(title: string, options: BibTeXGenerationOptions = DEFAULT_OPTIONS): BibTeXEntry {
  ...
  if (options.includeAbstract) {
    entry.abstract = `This is a placeholder abstract for the paper titled "${title}". ` +
 ...
 */
async function searchSemanticScholar(title: string, maxResults: number = 3): Promise<SemanticScholarPaperResponse[]> {
  try {
 ...
 * @returns BibTeX entry object
 */
function semanticScholarToBibTeX(paper: SemanticScholarPaperResponse, options: BibTeXGenerationOptions = DEFAULT_OPTIONS): BibTeXEntry {
  // Extract authors
 ...
 */
export async function generateBibTeXFromTitle(title: string, options: BibTeXGenerationOptions = DEFAULT_OPTIONS): Promise<string> {
  ...
  if (mergedOptions.useApi) {
    try {
      const results = await searchSemanticScholar(title, mergedOptions.maxResults);
 ...
 * @returns Promise resolving to a string with the BibTeX entry
 */
export async function generateBibTeXFromMarkdown(markdownContent: string, options: BibTeXGenerationOptions = DEFAULT_OPTIONS): Promise<string> {
