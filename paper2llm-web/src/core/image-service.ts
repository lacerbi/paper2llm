  ...
  async describeImage(
    ...
    try {
      ...
      try {
        ...
        if (apiError.response) {
          ...
          if (statusCode === 422) {
            ...
            console.error(`Vision API rejected request with 422 error: ${errorMessage}`);
      ...
      if (error instanceof ImageProcessingError && error.retryable) {
        console.log(`Retrying image ${image.id} description (attempt ${retryCount + 1} of ${this.maxRetries})`);
        // Wait before retry
    ...
    try {
      ...
      for (let i = 0; i < totalImages; i++) {
        ...
        try {
          ...
          console.error(`Error processing image ${image.id} (${errorType}):`, errorMessage);
